...
The table below shows us, how many times was some power grid congested.
...
```{r, echo=FALSE}
grouped2 <- Grouped[Grouped$Sum > 0,]
...
trend_data2 <- trend_data[trend_data$Time == "00:00:00" & trend_data$Day ==  1,]
# Define UI
                ...
                sidebarLayout(
                  sidebarPanel(
                    ...
                    selectInput(inputId = "name2", label = strong("Name of the Power Grid"),
                                choices = unique(trend_data2$Name2)[order(unique(trend_data2$Name2))],
...
server <- function(input, output, session) {
  ...
  observe({
    trend_data3 <- trend_data[trend_data$Time==input$time & trend_data$Day==input$day,]
    val <- input$name2
    # Control the value, min, max, and step.
    # Step size is 2 when input value is even; 1 when value is odd.
    updateSelectInput(session, "name2", choices = unique(trend_data3$Name2)[order(unique(trend_data3$Name2))], selected = input$name2)
  })
  
  trend_data2 <- reactive({trend_data[trend_data$Time == input$time & trend_data$Day ==  input$day,]})
